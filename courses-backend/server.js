...
app.get('/courses', (req, res) => {
    try {
        ...
        if (descriptionFilter) {
            ...
            courses = courses.filter(course => course.description.toLowerCase().includes(descriptionFilter.toLowerCase()));
        }
...
app.get('/courses/:id', (req, res) => {
    ...
    try {
        const data = loadData(); // Load the data from the JSON file
        const course = data.courses.find(courseItem => courseItem.id === courseId); // Find the course with the matching ID
        if (course) {
...
app.get('/courses/:id/users', (req, res) => {
    ...
    const enrolledusers = data.users.filter((user) =>
